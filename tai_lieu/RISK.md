...
| R-05 | ~~CI trên runner sạch **đỏ oan** vì thiếu ổ mạng/NAS/ffmpeg~~ | — | **ĐÃ ĐO 2026-09-11:** giấu `ffmpeg/` rồi chạy lại toàn bộ → **44/44 ĐẠT, 0 bỏ qua**. Sửa 3 bộ đòi ffmpeg đi kèm (`test_dong_goi`, `test_phien_ban`, `test_lan_dau`) sang bỏ qua **có khai lý do**, không hạ thấp phép kiểm trên máy dev. `chay_het.py` nay đọc lý do thật do bộ kiểm tự khai + có ngưỡng `MAX_BO_QUA` | **Đã xử lý** (ISSUE-011) |
...
- **R-05 đóng 2026-09-11 bằng số đo thật**, và số đo làm kế hoạch đơn giản đi hẳn: dự kiến ban đầu là **ba workflow + cache 195 MB ffmpeg**, đo xong thì chỉ cần **một job, không cần ffmpeg**. `grep "ffmpeg"` cho ra 20 file "phụ thuộc"; đo thật chỉ 3 file cần, và mỗi file chỉ ở vài phép kiểm lẻ. **Đo trước khi thiết kế.**
